...
public class Geonames extends Controller {
    ...
    private static JsonObject getGeonames(String ws) throws IOException {
	    ...
	    VirtualFile vf = VirtualFile.fromRelativePath("/cache/geonames-" + ws.replace('?', '_') + "-" + Lang.get() + ".json.gz");
	    if (!vf.exists()) {
           WS.HttpResponse response = WS.url("http://ws.geonames.org/" + ws + "&style=short&lang=" + Lang.get()).get();
    ...
    public static void zones(@Required String countryId) throws IOException {
	    if (!validation.hasErrors()) {
		    JsonObject zones = getGeonames("childrenJSON?geonameId=" + countryId);
