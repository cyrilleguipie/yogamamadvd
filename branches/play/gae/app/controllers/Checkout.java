...
public class Checkout extends Controller {
	...
	Cart cart = getCart();
	// FIXME: product code
	cart.add(productId.toString(), quantity, product.price);
	...
	for (String productIdString : cart.items.keySet()) {
	  // FIXME: product code
	...
	if (getCart().shipment != null) {
	    Map<String, List<PaymentGateway>> gateways =
		new HashMap<String, List<PaymentGateway>>();
	    for (String category : PaymentGateway.CATEGORIES) {
	...
	@Override
	    ...
	    return PaymentGateway.findByName(value) != null;
        }
    }
	...
	if (validation.hasErrors()) {
		...
		if (product != null) {
		    ...
		    // FIXME: product code
		    cart.add(productId.toString(), qty, product.price);
		}
	    }
