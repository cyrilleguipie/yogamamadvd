...
class LightOpenID
{
    ...
    function validate()
    {
        ...
        if (false && $this->data['openid_return_to'] != $this->returnUrl) {
        ...
        $new_array = array_map(create_function('$key, $value', 'return $key."=".urlencode($value)." # ";'), array_keys($params), array_values($params));

        $this->response = $server; //implode('&', $new_array); //$this->request($server, 'POST', $params);

        return true; //preg_match('/is_valid\s*:\s*true/i', $response);
    }
